...
#            pickle.dump(self.q_dist, output, pickle.HIGHEST_PROTOCOL)
    ...
    def fit_iterate(self, max_iter=500, pruning_crit = 1e-6, conv_crit = 1e-6, verbose = 0, Y_tst=[None], X_tst=[None]):
...
#                    self.L.append(self.update_bound())
    ...
    def compute_HL(self, Y_tst, X_tst, m_in=[0], m_out=1, th=0):
        ...
        if self.method[m_out] == 0:
            ...
            if self.SS[m_out]:
                ...
                Y_pred = np.argmax(self.predict(m_in, m_out, 0, X_tst), axis=1).astype(int)
    ...
    def compute_AUC(self, Y_tst, X_tst, m_in=[0], m_out=1, th=0):
        ...
        if self.method[m_out] == 0:
            if self.SS[m_out]:
                Y_pred = self.X[m_out]['mean'][self.n[m_out]:,:]
            else:
                [Y_pred, var] = self.predict(m_in, m_out, 0, X_tst)
            ...
            if self.SS[m_out]:
                Y_pred = self.t[m_out]['mean'][self.n[m_out]:,:]
            else:
                Y_pred = self.predict(m_in, m_out, 0, X_tst)
    ...
    def update(self, Y_tst=[None], X_tst=[None], verbose=0):
        """Update the variables of the model.
        ...
        for m in np.arange(self.m):
            ...
            if self.SS[m]:
                if self.method[m] == 0:
                    ...
                    q.XS[m]['LH'] = HXS
                if self.method[m] == 2:
                    HtS = np.sum(np.log(self.sigmoid(q.xi[m][self.n[m]:,:])) + self.X[m]['mean'][self.n[m]:,:] * self.t[m]['mean'][self.n[m]:,:] - (q.xi[m][self.n[m]:,:] + self.X[m]['mean'][self.n[m]:,:])*0.5)
                    if abs(HtS) == np.inf:
                        HtS = q.Z['LH']
                    q.tS[m]['LH'] = HtS
        EntropyQ = q.Z['LH']
